...
def create_project(request):
    try:
        ...
        # print(data)
        # print(type(data))
        tplt = """insert into project(pid, pname, uid, description, fuzzer_id, upload_t)
         values (%s, %s, %s, %s, %s, %s);"""
        data = (
            pid,
            req_data['name'],
...
def fake_list(request):
    ...
    for i, line in enumerate(cur.fetchall()):
        ...
        # avatar_url = "https://api.prodless.com/avatar.png?size={}".format(random.choice([48, 49, 50, 51]))
        avatar_url = "/autofuzz/randomAvatar/{}.png".format(random.choice(['a', 'b', 'c', 'd', 'e']))
        cover_url = 'https://gw.alipayobjects.com/zos/rmsportal/iZBVOIhGJiAnhplqjvZW.png'
        logo_url = avatar_url
        percent = compute_percent(line[-1])
        status = 'success' if percent >= 100 else 'active'
